...
pub fn one_one() {
    ...
    let map = init_map();
    let mut sum: u32 = 0;
    for (_i, s) in INPUT.iter().enumerate() {
...
fn parse_back_number(s: &str, map: &HashMap<&'static str, u32>) -> u32 {
    let number_text_regex = Regex::new(r"enin|thgie|neves|xis|evif|ruof|eerht|owt|eno").unwrap();
    return match number_text_regex.find(s) {
        None => s.parse::<u32>().unwrap(),
        Some(_m) => map.get(s).unwrap().to_owned()
    } 
