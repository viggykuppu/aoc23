...
pub fn one() {
    ...
    let mut hands: Vec<_> = input.lines().map(|line| {
        let caps: Vec<_> = hand_regex.captures_iter(line).map(|c| c.extract::<2>()).collect();
        let mut hand_chars: Vec<_> = caps.get(0).unwrap().1[0].chars().map(|c| {
            return if c == 'T' {
                'a'
            } else if c == 'J' {
                'b'
        ...
        hand::Hand::new(hand_chars, caps.get(0).unwrap().1[1].parse::<u32>().unwrap())
    }).collect();
    hands.sort();
...
pub fn two() {
    ...
    let mut hands: Vec<_> = input.lines().map(|line| {
        ...
        hand::Hand2::new(hand_chars, caps.get(0).unwrap().1[1].parse::<u32>().unwrap())
    }).collect();
    hands.sort();
    let total_points = hands.iter().enumerate().fold(0, |acc, (i, hand)| {
