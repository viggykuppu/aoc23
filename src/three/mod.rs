...
fn calculate_symbol_adjacencies(row_num: usize, range: Range<usize>, char_matrix: &Vec<Vec<char>>, part_number: u32, symbol_adjacency_map: &mut HashMap<(usize,usize), Vec<u32>>) {
    let dims = (char_matrix.len() as isize, char_matrix[0].len() as isize);
    let symbol_regex = Regex::new(r"[^\d\.\n]").unwrap();
    for i in range {
        for x in -1..=1 {
